...
RED.userSettings = (function() {
    ...
    var viewSettings = [
        ...
        },
        {
         ...
         // NOLEY - do this better just for mocks
        {
            title: "Appearance",
            options: [
                {
                    setting: "view-dark-galaxy-theme",
                    local: true,
                    label: "Dark Galaxy Theme",
                    toggle: true,
                    default: false,
                    onchange: function(val) {
                        var others = ["nr-theme-midnight","nr-theme-forest","nr-theme-sea","nr-theme-synth"];
                        var otherKeys = ["view-dark-midnight-theme","view-dark-forest-theme","view-dark-sea-theme","view-dark-synth-theme"];
                        if (val) {
                            document.documentElement.classList.add("nr-theme-dark");
                            others.forEach(function(c) { document.documentElement.classList.remove(c); });
                            otherKeys.forEach(function(k) { localStorage.setItem(k,"false"); document.getElementById("user-settings-"+k) && (document.getElementById("user-settings-"+k).checked = false); });
                        } else {
                            document.documentElement.classList.remove("nr-theme-dark");
                        }
                    }
                },
                {
                    setting: "view-dark-midnight-theme",
                    local: true,
                    label: "Midnight Theme",
                    toggle: true,
                    default: false,
                    onchange: function(val) {
                        var others = ["nr-theme-dark","nr-theme-forest","nr-theme-sea","nr-theme-synth"];
                        var otherKeys = ["view-dark-galaxy-theme","view-dark-forest-theme","view-dark-sea-theme","view-dark-synth-theme"];
                        if (val) {
                            document.documentElement.classList.add("nr-theme-midnight");
                            others.forEach(function(c) { document.documentElement.classList.remove(c); });
                            otherKeys.forEach(function(k) { localStorage.setItem(k,"false"); document.getElementById("user-settings-"+k) && (document.getElementById("user-settings-"+k).checked = false); });
                        } else {
                            document.documentElement.classList.remove("nr-theme-midnight");
                        }
                    }
                },
                {
                    setting: "view-dark-forest-theme",
                    local: true,
                    label: "Forest Theme",
                    toggle: true,
                    default: false,
                    onchange: function(val) {
                        var others = ["nr-theme-dark","nr-theme-midnight","nr-theme-sea","nr-theme-synth"];
                        var otherKeys = ["view-dark-galaxy-theme","view-dark-midnight-theme","view-dark-sea-theme","view-dark-synth-theme"];
                        if (val) {
                            document.documentElement.classList.add("nr-theme-forest");
                            others.forEach(function(c) { document.documentElement.classList.remove(c); });
                            otherKeys.forEach(function(k) { localStorage.setItem(k,"false"); document.getElementById("user-settings-"+k) && (document.getElementById("user-settings-"+k).checked = false); });
                        } else {
                            document.documentElement.classList.remove("nr-theme-forest");
                        }
                    }
                },
                {
                    setting: "view-dark-sea-theme",
                    local: true,
                    label: "Under the Sea Theme",
                    toggle: true,
                    default: false,
                    onchange: function(val) {
                        var others = ["nr-theme-dark","nr-theme-midnight","nr-theme-forest","nr-theme-synth"];
                        var otherKeys = ["view-dark-galaxy-theme","view-dark-midnight-theme","view-dark-forest-theme","view-dark-synth-theme"];
                        if (val) {
                            document.documentElement.classList.add("nr-theme-sea");
                            others.forEach(function(c) { document.documentElement.classList.remove(c); });
                            otherKeys.forEach(function(k) { localStorage.setItem(k,"false"); document.getElementById("user-settings-"+k) && (document.getElementById("user-settings-"+k).checked = false); });
                        } else {
                            document.documentElement.classList.remove("nr-theme-sea");
                        }
                    }
                },
                {
                    setting: "view-dark-synth-theme",
                    local: true,
                    label: "Synthwave Theme",
                    toggle: true,
                    default: false,
                    onchange: function(val) {
                        var others = ["nr-theme-dark","nr-theme-midnight","nr-theme-forest","nr-theme-sea"];
                        var otherKeys = ["view-dark-galaxy-theme","view-dark-midnight-theme","view-dark-forest-theme","view-dark-sea-theme"];
                        if (val) {
                            document.documentElement.classList.add("nr-theme-synth");
                            others.forEach(function(c) { document.documentElement.classList.remove(c); });
                            otherKeys.forEach(function(k) { localStorage.setItem(k,"false"); document.getElementById("user-settings-"+k) && (document.getElementById("user-settings-"+k).checked = false); });
                        } else {
                            document.documentElement.classList.remove("nr-theme-synth");
                        }
                    }
                }
            ]
    ...
    function init() {
        ...
        viewSettings.forEach(function(section) {
            section.options.forEach(function(opt) {
                if (!opt.local || !opt.onchange) { return; }
                var stored = localStorage.getItem(opt.setting);
                var val = (stored !== null) ? (stored === "true") : (opt.hasOwnProperty("default") ? opt.default : false);
                opt.onchange.call(opt, val);
            });
        });
